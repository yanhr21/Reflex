...
reason=Run this retry only inside a compute-node srun step from a tmux-held allocation.
EOF
  exit 30
fi

STAMP="${STAMP:-$(date +%Y%m%d_%H%M%S)}"
TAG="${TAG:-hardphase_h96_handoff_success_retry_${STAMP}_alloc${SLURM_JOB_ID}}"
...
export SCORER_PROGRESS_LOSS_WEIGHT="${SCORER_PROGRESS_LOSS_WEIGHT:-0.25}"
...
export SCORER_SCORE_PROGRESS_WEIGHT="${SCORER_SCORE_PROGRESS_WEIGHT:-0.25}"
export SCORER_SCORE_PROGRESS_DELTA_WEIGHT="${SCORER_SCORE_PROGRESS_DELTA_WEIGHT:-0.15}"
