...
# Full clean/dense SFT entry point. Use only after the 733 clean/dense
# condition root has a non-diagnostic clean_dense_preflight_summary.json with
# ready_for_overfit=true and after the short overfit gate has passed.
...
output_root=${OUTPUT_ROOT:-${ROOT}/experiments/world_model_task_rebinding/cosmos3/sft_full_episode_wam_fix3_v7_733_clean_dense_rgb_300step_fix1recipe_4gpu_${STAMP}}
expected_source_episodes=${EXPECTED_SOURCE_EPISODES:-733}
run_sft=${RUN_SFT:-true}
allow_clean_dense_full_sft=${ALLOW_CLEAN_DENSE_FULL_SFT:-false}
allow_live_query_coverage_gap_for_full_sft=${ALLOW_LIVE_QUERY_COVERAGE_GAP_FOR_FULL_SFT:-false}
...
data_parallel_shard_degree=${DATA_PARALLEL_SHARD_DEGREE:-4}
boundary=configuration-only dry run; no export, training, rendering, or eval is launched.
EOF
  exit 0
fi

if [[ "${ALLOW_CLEAN_DENSE_FULL_SFT:-false}" != "true" ]]; then
  cat >&2 <<'EOF'
refusing_clean_dense_full_sft=true
reason=Set ALLOW_CLEAN_DENSE_FULL_SFT=true only after clean/dense preflight and short overfit pass.
...
if [[ -z "${CONDITION_ROOT:-}" ]]; then
  cat >&2 <<'EOF'
refusing_clean_dense_full_sft=true
reason=CONDITION_ROOT must point at the approved 733 clean/dense condition root.
EOF
  exit 42
fi
...
ready_gate_args=(
  ...
  --output-json "$(dirname "${CLEAN_DENSE_PREFLIGHT_SUMMARY}")/clean_dense_preflight_summary_full_sft_entry_gate.json"
)
if [[ "${ALLOW_LIVE_QUERY_COVERAGE_GAP_FOR_FULL_SFT:-false}" == "true" ]]; then
  ready_gate_args+=(
    --allow-user-overridden-live-query-coverage-gap
    --override-note "2026-06-14 user override: run dense late299 full SFT first; record 58/173 live-query coverage gap as limitation, not launch blocker."
  )
fi
...
export OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT}/experiments/world_model_task_rebinding/cosmos3/sft_full_episode_wam_fix3_v7_733_clean_dense_rgb_300step_fix1recipe_4gpu_${STAMP}}"
...
export ALLOW_LIVE_QUERY_COVERAGE_GAP_FOR_FULL_SFT="${ALLOW_LIVE_QUERY_COVERAGE_GAP_FOR_FULL_SFT:-false}"
...
export MASTER_PORT="${MASTER_PORT:-50379}"
