...
COLLECTOR="${ROOT}/scripts/world_model/collect_dynamic_rgb_observation_smoke.py"
...

{
  ...
  echo "controller=official_demo_action_dynamic_observation"
  ...
  echo "allowed_losses=cosmos_dynamic_future,target_frame_readout,trajectory_consistency,uncertainty"
  echo "disallowed_losses=positive_dp_bc_from_failed_actions,final_method_evidence"
  ...
  echo "notes=B dynamic RGB smoke uses official demo actions plus dynamic target motion; not positive DP behavior data; requires human visual review."
} | tee "${MANIFEST}"
...
echo "dataset_smoke_status=dynamic_collection_in_progress" | tee -a "${MANIFEST}"
"${VENV}/bin/python" -u "${DEMO_COLLECTOR}" \
  --dataset-class B_dynamic_rgb_observation \
