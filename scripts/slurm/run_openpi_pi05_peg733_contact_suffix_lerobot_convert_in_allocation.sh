...
example=srun --jobid=<held_job_id> --ntasks=1 --cpus-per-task=8 --mem=80G bash scripts/slurm/run_openpi_pi05_peg733_contact_suffix_lerobot_convert_in_allocation.sh
EOF
  exit 30
fi
...
method_boundary=Official OpenPI LeRobot contact-suffix data preparation; no scorer-only branch and no custom intermediate model.
EOF
