...
reason=Run OpenPI norm stats and pi0.5 training only inside a compute-node srun step.
example=srun --jobid=<held_job_id> --ntasks=1 --gres=gpu:1 --cpus-per-task=16 --mem=120G bash scripts/slurm/run_openpi_pi05_peg733_train_in_allocation.sh
EOF
  exit 30
fi
...
SKIP_EXISTING_NORM_STATS="${SKIP_EXISTING_NORM_STATS:-false}"
...
if [[ -z "${DATASET_ASSET_ID:-}" ]]; then
  case "${CONFIG_NAME}" in
    ...
    pi05_maniskill_peg733_contact_suffix16_object17_video_nearcontact_20260626)
      DATASET_ASSET_ID="yanhongru/maniskill_peg733_openpi_contact_suffix16_object17_video_nearcontact_20260626"
      ;;
    *)
      DATASET_ASSET_ID=""
      ;;
...
schema=openpi_pi05_peg733_train_wrapper_v1
...
skip_existing_norm_stats=${SKIP_EXISTING_NORM_STATS}
...
if [[ "${SKIP_EXISTING_NORM_STATS}" == "true" && -f "${NORM_STATS_PATH}" ]]; then
  {
  ...
  if [[ -n "${OPENPI_PYTHON}" ]]; then
    "${OPENPI_PYTHON}" scripts/compute_norm_stats.py --config-name "${CONFIG_NAME}" \
      2>&1 | tee "${OUTPUT_ROOT}/compute_norm_stats.log"
  else
    ...
    "${UV_RUN_ARGS[@]}" scripts/compute_norm_stats.py --config-name "${CONFIG_NAME}" \
      2>&1 | tee "${OUTPUT_ROOT}/compute_norm_stats.log"
  fi
fi
