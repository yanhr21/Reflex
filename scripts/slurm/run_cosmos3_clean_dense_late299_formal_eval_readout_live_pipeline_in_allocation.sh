...
reason=Run this formal pipeline only inside a compute-node srun step. It waits for a current clean-dense checkpoint saved after the formal training-time boundary.
EOF
  exit 30
fi
...
PIPELINE_LOG_ROOT="${PIPELINE_LOG_ROOT:-${SFT_ROOT}/formal_eval_readout_live_pipeline_after_3h}"
...
mkdir -p "${PIPELINE_LOG_ROOT}"
{
  ...
  echo "boundary=formal current-run pipeline; selected checkpoint must have actual save timestamp after the 2-GPU/3-hour training floor"
...
bash "${ROOT}/scripts/slurm/watch_cosmos3_clean_dense_late299_iter300_readout_in_allocation.sh" \
  2>&1 | tee "${PIPELINE_LOG_ROOT}/02_formal_readout.log"
...
bash "${ROOT}/scripts/slurm/watch_cosmos3_clean_dense_late299_iter300_live_receding_panel_in_allocation.sh" \
  2>&1 | tee "${PIPELINE_LOG_ROOT}/03_formal_live_receding_panel.log"
