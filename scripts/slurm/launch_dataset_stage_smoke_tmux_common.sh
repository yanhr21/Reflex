...
EXCLUDE_NODES="${EXCLUDE_NODES:-server07,server28,server34,server36,server39,server43,server44,server46,server53,server58,server60,server63}"
NODELIST="${NODELIST:-}"
...
export FUTURE_RESIDUAL_GAIN="${FUTURE_RESIDUAL_GAIN:-}"
export MAX_FUTURE_RESIDUAL_M="${MAX_FUTURE_RESIDUAL_M:-}"
export PEG_DISTURB_FORCE_X="${PEG_DISTURB_FORCE_X:-}"
export PEG_DISTURB_FORCE_Y="${PEG_DISTURB_FORCE_Y:-}"
export PEG_DISTURB_FORCE_Z="${PEG_DISTURB_FORCE_Z:-}"
export PEG_DISTURB_DURATION_STEPS="${PEG_DISTURB_DURATION_STEPS:-}"
...
echo "motion_trigger_mode=${MOTION_TRIGGER_MODE:-default}" | tee -a "${LOG_FILE}"
echo "motion_trigger_threshold_m=${MOTION_TRIGGER_THRESHOLD_M:-default}" | tee -a "${LOG_FILE}"
echo "motion_trigger_min_step=${MOTION_TRIGGER_MIN_STEP:-default}" | tee -a "${LOG_FILE}"
echo "min_trigger_to_insert_steps=${MIN_TRIGGER_TO_INSERT_STEPS:-default}" | tee -a "${LOG_FILE}"
echo "future_tau_steps=${FUTURE_TAU_STEPS:-default}" | tee -a "${LOG_FILE}"
echo "future_residual_gain=${FUTURE_RESIDUAL_GAIN:-default}" | tee -a "${LOG_FILE}"
echo "max_future_residual_m=${MAX_FUTURE_RESIDUAL_M:-default}" | tee -a "${LOG_FILE}"
echo "peg_disturb_force=${PEG_DISTURB_FORCE_X:-default},${PEG_DISTURB_FORCE_Y:-default},${PEG_DISTURB_FORCE_Z:-default}" | tee -a "${LOG_FILE}"
echo "peg_disturb_duration_steps=${PEG_DISTURB_DURATION_STEPS:-default}" | tee -a "${LOG_FILE}"
