#!/usr/bin/env python3
"""Collect D-class future-frame cooperation teacher smoke data inside Slurm.

This collector uses ground-truth future target motion only to generate
teacher-only controller actions. It records legal `pd_ee_delta_pose` actions,
RGB video, and traces. These artifacts are not deployed-method evidence.
"""
...
def _tcp_position(base_env: Any) -> np.ndarray:
    ...
    if tcp is None:
        raise RuntimeError("PegInsertionSide-v1 agent has no tcp pose for teacher action generation")
...
def _build_motion_plan(
    *,
    ...
    horizon: int,
) -> dict[int, MotionCommand]:
    plan: dict[int, MotionCommand] = {}
...
def _future_target_position(
    *,
    motion_plan: dict[int, MotionCommand],
    initial_p: np.ndarray,
    step: int,
    tau: int,
) -> tuple[np.ndarray, str]:
    future_step = step + tau
    if future_step in motion_plan:
        return np.asarray(motion_plan[future_step].target_p, dtype=np.float64), "gt_future_motion_plan"
    if motion_plan:
        max_step = max(motion_plan)
        if future_step > max_step:
            return np.asarray(motion_plan[max_step].target_p, dtype=np.float64), "gt_future_motion_plan_clamped"
    return np.asarray(initial_p, dtype=np.float64).reshape(3), "static_initial_target"
...
def _teacher_action(
    *,
    ...
    desired_delta = np.asarray(target_p, dtype=np.float64).reshape(3) + approach_offset - tcp_p
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        for rollout_idx in range(args.num_rollouts):
            ...
            motion_plan = _build_motion_plan(
                ...
                initial_p=initial_p,
                initial_q=initial_q,
                horizon=args.max_episode_steps + args.future_tau_steps + 1,
            )
            ...
            for step in range(args.max_episode_steps):
                ...
                future_p, future_source = _future_target_position(
                    motion_plan=motion_plan,
                    initial_p=initial_p,
                    step=step,
                    tau=args.future_tau_steps,
                )
                ...
                action_rows.append(
                    {
                        ...
                        "future_target_source": future_source,
                        "future_target_p": future_p.astype(float).tolist(),
                        "approach_offset": approach_offset.astype(float).tolist(),
    ...
    trace_path = trace_dir / "future_teacher_trace.json"
    trace_payload = {
        "dataset_class": "D_future_frame_cooperation_teacher",
        "scenario": args.scenario,
        "seed": args.seed,
        "num_rollouts": args.num_rollouts,
        "future_tau_steps": args.future_tau_steps,
    ...
    loss_fields = manifest_fields("D_future_frame_cooperation_teacher")
...
def main() -> None:
    try:
        ...
        print(json.dumps({"future_frame_teacher_smoke_failed": str(exc)}, sort_keys=True), file=sys.stderr)
        raise
