#!/usr/bin/env python3
"""Generate corrected fix3 late-trigger dynamic PegInsertionSide demos.

This replaces the rejected kinematic overlay package. The target block remains
static while the robot grasps the peg and approaches the current hole, then
moves quickly in the real ManiSkill episode. The expert replans to the final
target pose and inserts the peg there. The exported review H5s keep the
301-frame / 300-action contract and are meant for rendered user approval before
any SFT.
"""
...
@dataclass
class Args:
    output_root: str = "experiments/world_model_task_rebinding/cosmos3/fix3_late_trigger_review60_20260610_6x10"
    paths_file: str = ""
    trajectory_name: str = "fix3_late_trigger_raw_pd_joint_pos"
    ...
    num_demos: int = 60
    max_attempts: int = 260
    seed: int = 970000
    total_video_frames: int = 301
    total_action_steps: int = 300
    motion_min_m: float = 0.09
    motion_max_m: float = 0.17
    final_x_min: float = -0.05
    final_x_max: float = 0.05
    final_y_min: float = 0.20
    final_y_max: float = 0.40
    move_steps_min: int = 10
    move_steps_max: int = 24
    continuous_move_steps_min: int = 28
    continuous_move_steps_max: int = 52
    ...
    final_insert_refine_steps: int = 1
        ...
        for refine_idx in range(num_refines):
            ...
            if bool(line_info["wall_collision_risk"]):
                ...
                if (
                    ...
                    or float(line_info["centerline_yz_max_m"]) > float(args.max_constrained_insert_raw_line_yz_m)
                ):
                    return False, {
...
def _reject(reason: str, **details: Any) -> None:
    print(
        json.dumps(
            _jsonable({"event": "fix3_late_trigger_reject", "reason": reason, **details}),
            sort_keys=True,
        ),
        flush=True,
    )
...
def _sample_final_pose(
    *,
    ...
    away_ref = np.asarray(initial_p[:2] - peg_head_xy, dtype=np.float32)
    if float(np.linalg.norm(away_ref)) < 1e-5:
        away_ref = np.asarray([1.0, 0.0], dtype=np.float32)
    away_ref = away_ref / max(float(np.linalg.norm(away_ref)), 1e-6)

    for _ in range(1000):
        mag = float(rng.uniform(args.motion_min_m, args.motion_max_m))
        if scenario == "hole_late_fast_shift":
            mag = float(rng.uniform(max(args.motion_min_m, 0.12), args.motion_max_m))
        if scenario == "hole_late_continuous_insert":
            mag = float(rng.uniform(args.motion_min_m, min(args.motion_max_m, 0.15)))
        angle = float(rng.uniform(-np.pi, np.pi))
        direction = np.asarray([np.cos(angle), np.sin(angle)], dtype=np.float32)
        # Reject target motion that moves the hole toward the waiting peg.
        if float(np.dot(direction, away_ref)) < 0.10:
            continue
        final_xy = initial_p[:2] + direction * mag
    ...
    raise RuntimeError("failed to sample a late target final pose inside domain and away from peg")
...
def _standardize_one(
    *,
    ...
    with h5py.File(out_h5, "w") as h5:
        ...
        _write_dataset(perturb, "peg_delta_applied", np.zeros((int(args.total_action_steps), 3), dtype=np.float32))
...
def main() -> None:
    ...
    target_counts = {scenario: int(args.num_demos) // len(SCENARIOS) for scenario in SCENARIOS}
    remainder = int(args.num_demos) - sum(target_counts.values())
    for scenario in SCENARIOS[:remainder]:
    ...
    try:
        for attempt_i in range(int(args.max_attempts)):
            if len(accepted) >= int(args.num_demos):
                break
            underfilled = [s for s in SCENARIOS if sum(1 for r in accepted if r["scenario"] == s) < target_counts[s]]
            if not underfilled:
                break
