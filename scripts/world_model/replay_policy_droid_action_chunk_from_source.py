#!/usr/bin/env python3
"""Replay one Cosmos Policy-DROID action chunk from a source H5 prefix state."""
...
from export_cosmos3_candidate_outcome_labels import run_dp_rollout_continuability_label  # noqa: E402
...
from run_cosmos3_live_receding_loop import (  # noqa: E402
    initialize_history_from_source,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--candidate-name", default="cosmos_policy_droid_10step")
...
def main() -> int:
    ...
    label = replay_bank_candidate(
        ...
        base_env=base_env,
        stack=stack,
        snapshot_state=env_states[prefix_frame],
        ...
        candidate_meta={
            "candidate_source": "cosmos_policy_droid_live_prefix",
    ...
    label.update(
        {
            "schema": "policy_droid_source_prefix_action_replay_v1",
            "source_h5": str(source_h5),
            "action_chunk_json": str(action_path),
            "sample_output_json": str(chunk.get("sample_output_json", "")),
            ...
            "boundary": (
                "Label-only replay of one Cosmos Policy-DROID action chunk "
                "from the matching source H5 prefix state. This is not live "
                "controller evidence until replayed from dynamic saved/live "
                "states with visual review."
            ),
        }
    )
    write_json(output_root / "policy_droid_action_replay_label.json", label)
    summary = {
        "label_path": str(output_root / "policy_droid_action_replay_label.json"),
        "source_h5": str(source_h5),
        "action_chunk_json": str(action_path),
    ...
    write_json(output_root / "policy_droid_action_replay_summary.json", summary)
